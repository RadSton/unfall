package io.radston12.unfall.commands.map;
...
public class Map {
    public static int execute(CommandContext<CommandSourceStack> command) {
        ...
        try { // "https://image.spreadshirtmedia.net/image-server/v1/products/T1459A839PA4459PT28D163573998W10000H8860/views/1,width=550,height=550,appearanceId=839,backgroundColor=F2F2F2/tom-und-jerry-freunde-sticker.jpg"
            ...
            if (a.length == 1) {
                if (a[0] == -1) {
                    p.sendSystemMessage(Component.literal("Fehler: Es werden nur quadratische Bilder unterstützt!"));
                    return 0;
                } else if (a[0] == -2) {
                    p.sendSystemMessage(Component.literal("Fehler: Dieses Bild konnte nicht geladen werden!"));
                    return 0;
                }
            }
            ...
            e.printStackTrace();
            p.sendSystemMessage(Component.literal("Fehler: Ein unbekannter Fehler ist aufgetreten!"));
        }
