...
public class CompressedBasicBlock extends BaseCompressedBlock {
    ...
    public void generateBlockStateProviderData(ModBlockStateProvider provider, RegistryObject<? extends Block> block) {
        ...
        provider.simpleBlock(block.get(), provider.models().withExistingParent(getRegistryName(), new ResourceLocation(RedDefenseMod.MOD_ID, "block/compressed_cube")).texture("all", all)); //cubeAll(ForgeRegistries.BLOCKS.getKey(block.get()).getPath(), provider.blockTexture(vanillaBlock)));
